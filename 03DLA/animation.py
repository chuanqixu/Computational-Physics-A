...
frequency = int(num/100)

#画散点图
fig, ax = plt.subplots()
plt.scatter(points[0, 0], points[0, 1], s=2, c='r')
...
#定义动态图函数
def animat(i):
    plt.scatter(points[i*frequency:i*frequency+frequency, 0], points[i*frequency:i*frequency+frequency, 1], s=0.5, c='b')

#生成动态图
ani = animation.FuncAnimation(fig, animat, frames=int(num), interval=1, blit=False)
...
ani.save('%dParticlesAnimation.mp4' % num, writer=writer)